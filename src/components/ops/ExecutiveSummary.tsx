...
export const ExecutiveSummary = ({ rows, summary, inputs }: ExecutiveSummaryProps) => {
  ...
  return (
    <section className="ops-panel overflow-hidden animate-slide-up flex flex-col md:flex-row md:items-center justify-between p-4 gap-3">
      ...
      {/* Lado direito: Status de capacidade integrado */}
      <div
        className={cn(
          "rounded-lg border px-4 py-2.5 flex items-center gap-2.5 md:max-w-2xl",
          hasRisk ? "border-warning/20 bg-warning/5" : "border-success/20 bg-success/5"
        )}
      >
        {hasRisk ? (
          <>
            ...
            <div className="text-xs leading-normal">
              <span className="font-semibold text-warning-foreground">
                Atenção: {summary.riskMonths.length} mês(es) com gap de capacidade.
              </span>{" "}
              <span className="text-muted-foreground">
                Necessário admitir <span className="font-medium text-foreground">{formatInt(summary.hiresYear)} agentes</span>
          ...
          <>
            ...
            <span className="text-xs font-medium text-success-foreground">
              Capacidade balanceada: Headcount atual é suficiente para atender a demanda.
            </span>
          </>
        )}
      </div>
